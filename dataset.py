...
def get_noised(img, sigma): 
    dim = np.shape(img)
    sigma_random = sigma
    noise = sigma_random * np.random.randn(dim[0], dim[1], dim[2]) 
...
class Dataset_cave_train(data.Dataset):
	def __init__(self, file_train_path, sigma):
	...
	def __getitem__(self, index):
		...
		img_noised = get_noised(img, self.sigma)

		data_hsi = self.transforms(img_noised.transpose([1, 2, 0]))
		label = self.transforms(img.transpose([1, 2, 0]))
		
		return data_hsi.unsqueeze(0), label.unsqueeze(0)

	def __len__(self):
		return 100 * len(self.tif_list) # num of patchs is 50 in one raw train image
